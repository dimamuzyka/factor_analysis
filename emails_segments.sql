...
where total_clicks >= 2
and active_date >= 2
and ea.customer_id <> 30338103
),
mail_event as (
select cast(customer_id as int) as customer_id
from yc_hive.masterdata_workflow.mail_event
where event = 'deliv' and dt > date'2024-01-01'
)
select distinct email
from customers c
    join yc_hive.masterdata_general.customer_v2 cv on c.customer_id = cv.id
    join mail_event me on c.customer_id = me.customer_id
